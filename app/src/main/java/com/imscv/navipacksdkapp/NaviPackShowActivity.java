...
public class NaviPackShowActivity extends Activity implements Runnable {
    ...
    private void initView()
    {
        ...
        btnMoveback.setOnClickListener(btnClickListener);
            ...
            switch(msgType)
            {
                ...
                case NaviPackType.DEVICE_MSG_TYPE_NAVIGATION:
                    ...
                    if(msgCode == NaviPackType.CODE_TARGET_REACH_POINT) {
                        ...
                        if(isMoveBack && --times > 0) {
//                            if(statusReg.posX == endX && statusReg.posY == endY){
//
//                            }
                            if(times % 2 == 1) {
                                Log.i("Catch", "moveback1");
                                int[] posX = new int[1];
                                int[] posY = new int[1];
...
//                        mapSurfaceView.setZOrderOnTop(false);
                ...
                case R.id.btnMoveBack:
                    ...
                    isMoveBack = true;
                    times = 20;
                    touchPoint = mapSurfaceView.getTouchedPoint();//获取图中我们所选取的点
                    targetPoint = PosTransform.pixToPoint(touchPoint, mapData);//转换到世界坐标系
                    ...
                    startX = statusReg.posX;
                    startY = statusReg.posY;
                    endX = targetPoint.x;
                    endY = targetPoint.y;
                        posX[0] = endX;
                        posY[0] = endY;
...
//                    for(int i=0;i<20;) {
//                        posX[i] = targetPoint.x;
//                        posY[i] = targetPoint.y;
//                        posX[i+1] = statusReg.posX;
//                        posX[i+1] = statusReg.posY;
//                        i+=2;
//                    }


                    Log.i("Catch", "start:" + startX + " start:" + startY);
                    Log.i("Catch", "end:" + endX + " end:" + endY);
                    mNaviPack.setTargets(mHandlerId, posX, posY, 1, 0);
